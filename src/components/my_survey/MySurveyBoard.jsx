    ...
    <div>
      {survey &&
        survey.map((data) => (
          <div
            key={data.surveyId}
            className="overflow-hidden border-t-2 border-main_color w-full"
          >
            <Link
              to={
      ...
      {showModal && (
        <CloseSurveyModal onClose={handleSurveyClose} onConfirm={closeModal} />
      )}
    </div>
  );
}
