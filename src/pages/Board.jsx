  ...
  useEffect(() => {
    if (data) {
      if (status[0] == "전체" && status[1] == "전체") {
        ...
        setboardData(
          data.filter(
            (item) => item.category == status[1] && item.status == status[0]
          )
        );
      }
    }
    ...
    <div>
      <nav>
        <div>
          <ul className="flex space-x-12 ml-left my-7">
            {["전체", "IT", "교육", "경제", "사회", "문화"].map(
              (item, index) => (
                <li
                  key={index}
                  onClick={() => handleItemClick(item)}
                  className={`click_highlight ${
                    status[1] == item ? "text-highligth" : ""
                  }`}
                >
                  {item}
                </li>
        ...
        <div className="flex justify-between">
          ...
          <ul className="flex space-x-5 ml-left my-7">
            <button
              ...
              className={`${
                status[0] === "전체" ? "select-list-item" : "common-list-item"
              }`}
            >
              전체
            </button>
            <button
              onClick={() => handleStatus("uncomplete")}
              className={`${
                status[0] === "uncomplete"
                  ? "select-list-item"
                  : "common-list-item"
      ...
      <main>
        <div className="w-screen flex flex-col items-center">
          {boardData &&
            boardData.map((item) => <BoardItem key={item.id} data={item} />)}
